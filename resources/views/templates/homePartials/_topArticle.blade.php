        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="relative h-[500px] rounded-xl overflow-hidden group">
                    <a href="{{ route('pages.articleDetail', ['slug' => $mainArticle->slug]) }}">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-8">
                            <span class="article-tag text-sm px-4 py-1.5 bg-accent text-white rounded-full mb-4 inline-block transform hover:scale-105 transition-transform">{{ $mainArticle->category->name }}</span>
            ...
            <div class="flex flex-col lg:gap-6 md:flex-row lg:flex-col">
                @foreach($secondaryArticles as $key => $article)
                    <div class="relative h-[240px] md:flex-1 rounded-xl overflow-hidden group {{ $key == 1 ? 'mt-6 md:mt-0 lg:mt-0' : '' }}">
